...
class RetryHandlerOption(RequestOption):
    ...
    def __init__(self, delay: int = DEFAULT_DELAY, max_retries: int = DEFAULT_MAX_RETRIES,
                 should_retry: bool = DEFAULT_SHOULD_RETRY) -> None:
        if delay > self.MAX_DELAY and max_retries > self.DEFAULT_MAX_RETRIES:
            raise ValueError(f'MaxLimitExceeded. Delay and MaxRetries should not be more than ${self.MAX_DELAY} and ${self.MAX_MAX_RETRIES}')
        elif delay > self.MAX_DELAY:
            raise ValueError(f'MaxLimitExceeded. Delay should not be more than ${self.MAX_DELAY}')
        elif max_retries > self.MAX_MAX_RETRIES:
            raise ValueError(f'MaxLimitExceeded. MaxRetries should not be more than ${self.MAX_MAX_RETRIES}')
        elif delay < 0 and max_retries < 0:
