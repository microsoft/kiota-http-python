...
class RedirectHandlerOption(RequestOption):
    ...
    def __init__(self, max_redirect: int = DEFAULT_MAX_REDIRECT, 
                 should_redirect: bool = True, 
                 allow_redirect_on_scheme_change: bool = False ) -> None:
        
        if max_redirect > self.MAX_MAX_REDIRECT:
            raise ValueError("MaxLimitExceeded. Maximum value for max_redirect property exceeded")
        
        if max_redirect < 0:
            raise ValueError("MaxLimitExceeded. Negative value for max_redirect property is invalid")
